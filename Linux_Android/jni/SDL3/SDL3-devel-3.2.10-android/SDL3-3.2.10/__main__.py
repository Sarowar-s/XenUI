...
"""
Create a SDL3 SDK prefix from an Android archive
This file is meant to be placed in a the root of an android .aar archive

Example usage:
```sh
python SDL3-3.2.10.aar -o /usr/opt/android-sdks
cmake -S my-project \
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert a SDL3 Android .aar archive into a SDK",
        allow_abbrev=False,
    )
    parser.add_argument("--version", action="version", version="SDL3 3.2.10")
    parser.add_argument("-o", dest="output", type=pathlib.Path, required=True, help="Folder where to store the SDK")
    args = parser.parse_args()

    print(f"Creating a SDL3 SDK at {args.output}...")
